...
Sin embargo, se recomienda *no* acceder en línea, sino instalar Lean en
su computadora directamente. Así puede guardar su progreso y en un
futuro tal vez trabajar con teoremas más complejos.

Esta primera hoja discute las bases de la lógica en Lean. La lógica es la
base de la matemática moderna (axiomática).

Si bien el estudio de la lógica puede ser complejo e interesante por su
cuenta, en este minicurso no vamos a profundizar en temas de lógica
matemática. La idea de este primer módulo es aprender sólo las bases de
la lógica mínimas necesarias para usar Lean y demostrar teoremas de
...
matemáticas como "2 es el único número primo par" o incluso "La parte
real de todo cero no trivial de la función zeta de Riemann es `1/2`".

En la lógica no importa lo que estas proposiciones representan. Lo que
importa son sus relaciones, como implicación.  En las matemáticas,
generalmente escribimos `P ⇒ Q` para decir que `P` implica `Q`. En Lean
se usa la flecha sencilla, `P → Q`, por razones que van más allá de este
minicurso (tiene que ver con un concepto llamado la correspondencia de
Curry-Howard, pero no profundizaremos en esto).
...
equivalente a decir `P → false`. El origen de esto es el concepto en
lógica de "Ex falso quodlibet", de la falsedad se puede deducir
cualquier cosa. Entonces una proposicicón `P` es falsa si (y sólo si)
...
demostrando la *implicación* `false → Q`, y no la proposición `Q` sola.
