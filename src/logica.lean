/- Este documento ha sido escrito completamente como un archivo de Lean.
   ...
   detalladas instrucciones, en inglés, de como instalar Lean 3 en
   diferentes sistemas operativos:

   https://leanprover-community.github.io/get_started.html

   Se recomienda enfáticamente participar activamente en este curso: no
   sólo escuche, ¡pruebe también!

   Si de verdad intentó instalar Lean y no pudo por alguna razón, una
   opción alternativa es ver esta hoja en línea:
   ...
   Sin embargo, se recomienda *no* acceder en línea, sino instalar Lean
   en su computadora directamente. Así puede guardar su progreso y en un
   futuro tal vez trabajar con teoremas más complejos.

   Esta primera hoja discute las bases de la lógica en Lean. La lógica
   es la base de la matemática moderna (axiomática).

   Si bien el estudio de la lógica puede ser complejo e interesante por
   su cuenta, en este minicurso no vamos a profundizar en temas de
   lógica matemática. La idea de este primer módulo es aprender sólo las
   bases de la lógica mínimas necesarias para usar Lean y demostrar
   teoremas de otras ramas de la matemática.
   ...
   Antes de empezar con definiciones, creamos un espacio de nombres para
   este curso, smf22. Este espacio lo usamos sólo para no tener
   problemas si usamos el mismo nombre que algo que ya exista. -/

namespace smf22

/- En la lógica los objetos principales de interés son
   "proposiciones". Una proposición es un enunciado o una aseveración,
   generalmente de una verdad demostrada o asumida, pero también de una
   falsedad. A Lean le decimos que `P` es una proposición con la
...
/- Esta declaración está diciéndole a Lean que tenemos tres variables,
   `P`, `Q` y `R`. Estas tres variables representan
   proposiciones. Podrían ser proposiciones como "todos los hombres son
   mortales" o "Sócrates es un hombre", ejemplos clásicos de la lógica,
   pero también proposiciones matemáticas como "2 es el único número
   primo par" o incluso "La parte real de todo cero no trivial de la
   función zeta de Riemann es `1/2`".

   En la lógica no importa lo que estas proposiciones representan. Lo
   que importa son sus relaciones, como implicación.  En las
   matemáticas, generalmente escribimos `P ⇒ Q` para decir que `P`
   implica `Q`. En Lean se usa la flecha sencilla, `P → Q`, por razones
   que van más allá de este minicurso (tiene que ver con un concepto
   llamado la correspondencia de Curry-Howard, pero no profundizaremos
   en esto).
...
/- La táctica `intro` se puede usar para introducir varias hipótesis a
   ...
   nuestro *objetivo* usando una suposición. Note cómo esto nos sirve
   para razonar "para atrás":
...
/- Al usar `have` le podemos dar nombre a una conclusión que tenemos, en
   el estilo de razonamiento "hacia adelante". Sin embargo, trabajando
   en Lean preferimos la otra variante, reduciendo el objetivo.
...
/- Le decimos a Lean que no tenemos una demostración usando `sorry`,
   ...
   preocupe, sin embargo, Lean advierte cuando un teorema ha sido
...
/- Hemos visto la implicación lógica. El siguiente concepto central que
   ...
   La negación de `P` se escribe `¬ P`. En Lean, esto es *por
   definición* equivalente a decir `P → false`. El origen de esto es el
   concepto en lógica de "Ex falso quodlibet", de la falsedad se puede
   deducir cualquier cosa. Entonces una proposicicón `P` es falsa si (y
   sólo si) podemos deducir falso de ella. Esto nunca pasa con una
   proposición verdadera, o si no nuestros axiomas serían
   inconsistentes.
...
/- Hemos escrito `↔` en vez de `→`, para decir "si y sólo si". `P ↔ Q`
   ...
   Para demostrarlo usamos una nueva táctia, `refl`, que quiere decir
   que por "reflexividad" (de la equivalencia). Básicamente le estamos
   diciendo esto es verdad porque ambos lados dicen lo mismo.
...
/- Esta demostración ocupa una tactica 'exfalso' para cambiar el
   objetivo a "false" (falso). Podemos hacer esto porque (asumimos que)
   a partir de nuestros axiomas no se puede demostrar falso.

   Entonces en este caso podemos deducir cualquier cosa, pero obviamente
   la premisa (falso) nunca se cumplirá. Recuerde que aquí estamos
   demostrando la *implicación* `false → Q`, y no la proposición `Q`
   sola.

   A continuación vamos a cubrir conjunciones (el "y" lógico). En Lean,
   igual que en lógica, escribimos una conjunción con el símbolo `∧`.

   En Lean, la hipótesis `P ∧ Q` es equivalente a tener dos hipótesis,
   `P` y `Q`. Podemos separarlas usando la táctica `cases`: -/
...
/- Igual que con la conjunción, es diferente cómo demostrar
una disyunción a cómo usarla en una demostración: -/
...
/- La táctica `left` (izquierda en inglés) se usa para escoger la parte
   izquierda de una disyunción. Equivalentamente, podemos demostrar
   escogiendo la derecha con la táctica `right`. Intente demostrar lo
   siguiente: -/
...
/- ¿Cómo usamos una disyunción en una hipótesis? Igual que vimos
   anteriormente, distinguiendo diferentes casos con la táctica `cases`. -/
...
/- ¿Cuál es la diferencia entre el uso de `cases` aquí en disyunción y
   anteriormente en una conjunción?
...
/- Finalmente, regresamos al principio del tercero excluido. ¿Puede
   demostrar la otra dirección de la doble negación que vimos
   anteriormente?
...
/- Aquí usamos `have` para incluir la hipótesis que obtenemos del axioma
   explícitamente en nuestra demostración: -/
